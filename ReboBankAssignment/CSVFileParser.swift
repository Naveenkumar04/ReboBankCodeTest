...
// MARK: Parser
public class CSVFileParser: NSObject {
    ...
    public init(with string: String, separator: String = ",", headers: [String]? = nil) {
        var parsedLines = CSVFileParser.records(from: string.replacingOccurrences(of: "\r\n", with: "\n")).map { CSVFileParser.cells(forRow: $0, separator: separator) }
    ...
    internal static func cells(forRow string: String, separator: String = ",") -> [String] {
        return CSVFileParser.split(separator, string: string).map { element in
